...
import { Coordinate, MarbleSolitaire } from "./marble-solitaire";
...
export class BoardSpace extends React.Component<BoardSpaceProps> {
  ...
  render() {
    ...
    if (this.props.id < 0) {
      ...
      classes += " empty"; 
    } else if (this.props.game.selctedMarble.row === this.props.coor.row && this.props.game.selctedMarble.col === this.props.coor.col) {
      // a selected marble
      classes += " marble";
      ...
      // empty space
      classes += (this.props.coor.row + this.props.coor.col) % 2 === 0 ? " even" : " odd";
    }

    return (
      <div onClick={this.handleClick.bind(this)} id={this.props.id.toString()} className={classes} style={style}>
        <p>{this.props.id}</p>
      </div>
    );
  }
